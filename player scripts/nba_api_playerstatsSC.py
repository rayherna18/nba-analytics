...
if response.status_code == 200:
    ...
    # Print the first player_stat
    if player_stats_data:
        print("First player_stat:")
        print(player_stats_data[0])

    # Create a cursor to execute SQL queries
    cursor = cnxn.cursor()

    # Define the table for player stats
    player_stats_table = 'PLAYERSTATS_SC'
    cursor.execute(f'''
    ...
    BEGIN
        CREATE TABLE {player_stats_table} (
            ...
            GameDate DATETIME,  -- Changed to DATETIME for storing StartDate
            Points INT,
            Position VARCHAR(10),
            Minutes VARCHAR(5),
            FGM INT,
    ...
    for player_stat in player_stats_data:
        if player_stat.get('player', {}).get('id') == player_id:
            ...
            # Retrieve StartDate from NBA_GAMES2023 table
            ...
            cursor.execute(f'''INSERT INTO {player_stats_table} VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)''', (
                ...
                game_date_formatted,  # Insert formatted game date
